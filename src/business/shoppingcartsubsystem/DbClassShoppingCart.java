...
public class DbClassShoppingCart implements DbClass, DbClassShoppingCartForTest {
	...
	private void buildSaveCartQuery() {
		query = "INSERT INTO shopcarttbl (shopcartid, custid,shipaddress1, "
				...
				+ "totaltaxamount, totalamountcharged) " + "VALUES (NULL, "
				+ custProfile.getCustId()
				+ ", '"
	...
	ShoppingCartImpl retrieveSavedCart(CustomerProfile custProfile)
	...
	private void populateTopLevelCart(ResultSet rs) throws DatabaseException {
		...
		try {
			if (rs.next()) {
				...
				shippingAddress = CustomerSubsystemFacade.createAddress(
						shipStreet, shipCity, shipState, shipZip, true, false);
				...
				billingAddress = CustomerSubsystemFacade.createAddress(
						billStreet, billCity, billState, billpZip, false, true);
				...
				creditCard = CustomerSubsystemFacade.createCreditCard(name,
						num, type, exp);

				// load cart
				cartImpl.setCartId((new Integer(rs.getInt("shopcartid"))
						.toString()));
		...
		try {
			while (rs.next()) {
				cartItem = new CartItemImpl(rs.getInt("shopcartid"),
						rs.getInt("productid"), rs.getInt("cartitemid"),
						makeString(rs.getInt("quantity")),
						makeString(rs.getDouble("totalprice")), true);
	...
	public ShoppingCart readSavedCart(CustomerProfile customerProfile) throws DatabaseException{
		return retrieveSavedCart(customerProfile);
	}
